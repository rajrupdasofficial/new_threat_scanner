...
# Step 2: Preprocess Data using only TensorFlow
def preprocess_data(text_list, cwe_name_list, max_words=10000, max_len=300):
    # Tokenize text using TensorFlow Tokenizer
    ...
    return X, y_one_hot, tokenizer, label_to_int, num_classes
...
# Step 4: Build Deep Model using TensorFlow
def build_deep_model(input_dim, input_length, num_classes):
    model = tf.keras.Sequential([
        tf.keras.layers.Embedding(input_dim=input_dim, output_dim=256, input_length=input_length),
...
if __name__ == "__main__":
    ...
    X, y, tokenizer, label_to_int, num_classes = preprocess_data(text_list, cwe_name_list)
    ...
    model = build_deep_model(input_dim=10000, input_length=300, num_classes=num_classes)
    model.summary()
    ...
    model.save('deep_vuln_model.keras')
